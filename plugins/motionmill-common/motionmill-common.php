...
{
	class MM_Common extends MM_Plugin
	{
		...
		{
			$options = $this->_('MM_Settings')->get_option( 'motionmill_common' );
	...
	// registers plugin
	function motionmill_plugins_add_common($plugins)
	{
		array_push($plugins, 'MM_Common');

		return $plugins;
	}
	
	add_filter( 'motionmill_plugins', 'motionmill_plugins_add_common', 5 );
}
