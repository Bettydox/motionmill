...
if ( ! class_exists('MM_Settings') )
{
	class MM_Settings extends MM_Plugin
	{
		...
		public function on_init()
		{
			...
			foreach ( apply_filters( $this->page_slug . '_pages', array() ) as $data )
			{
				...
				(
					...
					'title'       => $data['id'],
					'description' => '',
					'option_name' => stripos( $data['id'] , 'motionmill_' ) !== 0 ? 'motionmill_' . $data['id'] : $data['id'] // makes sure the option name has following format: 'motionmill_{page_id}'
				), $data);
			}
			...
			{
				if ( ! is_array($data) || empty($data['id']) || empty($data['page']) )
					continue;
		...
		public function on_admin_init()
		{
			...
			{
				add_settings_section( $section['id'], $section['title'], create_function('$a', 'echo "' . $section['description'] . '";'), $section['page'] );
			}
		...
		public function on_sanitize_input_field($input, $method, $field)
		{
			switch ( $method )
			{
				case 'trim' :
		...
		public function form_checkbox($args = array())
		{
			...
			$options['extra'] .= checked( isset($args['value']) && $args['value'] == '1', '1', false );
			$options['value'] = '1';

			printf( '<input type="checkbox" id="%s" class="%s" name="%s" value="%s" %s />', esc_attr($options['id']), esc_attr($options['class']), esc_attr($options['name']), esc_attr($options['value']), $options['extra'] );
		...
		{
			$this->form_textfield(array_merge($args, array
			(
